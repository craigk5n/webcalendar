...
if ( empty ( $error ) ) {
  ...
  if ( empty ( $error ) && ! dbi_execute ( 'INSERT INTO webcal_entry ( cal_id, '
         ...
         . 'cal_due_date, cal_due_time, cal_mod_date, cal_mod_time,
  ...
  for ( $i = 0; $i < $site_extracnt && empty ( $error ); $i++ ) {
    ...
    $value = $$extra_name;
    // echo 'Looking for ' . $extra_name . '... value = ' . $value .
    // ' ... type = ' . $extra_type . '<br />' . "\n";
    $sql = '';
    $query_params = array ();
    ...
    if ( strlen ( $extra_name ) || $extra_type == EXTRA_DATE ) {
      if ( $extra_type == EXTRA_URL || $extra_type == EXTRA_EMAIL ||
        ...
        if ( ! empty ( $extra_email ) ) {
          $value = ( $extra_type == EXTRA_RADIO
            ? $extra_arg1[$value] : $value );
      ...
      if ( strlen ( $ext_names[$i] ) ) {
        ...
        if ( $EXTERNAL_NOTIFICATIONS == 'Y' && $SEND_EMAIL != 'N' &&
          ...
          if ( ( ! $newevent && $EXTERNAL_UPDATES == 'Y' ) || $newevent ) {
            ...
            $ext_names[$i] = trim ( preg_replace ( '/\[[\d]]/', '',
                $ext_names[$i] ) );
