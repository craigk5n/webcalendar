...
$participants = $WC->getPOST ( 'selectedPart' );
...
$entry_date = parseDate ( $WC->getPOST ( 'entry_date' ) );
$entry_month = $entry_date[0];
$entry_day = $entry_date[1];
$entry_year = $entry_date[2];

$entry_changed = $WC->getPOST ( 'entry_changed' ) == 'Y' ? true : false;
...
if ( $timetype != 'T' ) {
  //store as user's timezone
  $eventstart = gmmktime ( $entry_hour, $entry_minute, 0, $entry_month, $entry_day, $entry_year );
} else {
  $eventstart = mktime ( $entry_hour, $entry_minute, 0, $entry_month, $entry_day, $entry_year );
}
...
if ($eType != 'task' ) {
  if ( $timetype != 'T' ) { 
    $eventstop = gmmktime ( $eventstophour, $eventstopmin, 0, $entry_month, $entry_day, $entry_year );
  } else {
    $eventstop = mktime ( $eventstophour, $eventstopmin, 0, $entry_month, $entry_day, $entry_year );
  }
...
if ( getPref ( '_ENABLE_REPEATING_FIELD' ) ) {
  ...
  if ( $rpt_type == 'monthlyByDay' && empty ( $rpt_mode ) &&
    empty ( $byday ) ) {
    $byday = ceil( $entry_day / 7 ) . $WC->byday_names[ date ( 'w', $eventstart ) ];
  }
...
if ( empty ( $error ) ) {
  ...
  if ( $do_override ) {
    $sql = 'INSERT INTO webcal_entry_exceptions ( cal_id, cal_date, cal_exdate ) ' .
      'VALUES ( ?, ?, ? )';
  ...
  if ( ! dbi_execute ( 'DELETE FROM webcal_entry_exceptions WHERE cal_id = ?', 
    array( $eid ) ) ) {
      $error = $dberror . dbi_error ();
  }
    // add repeating info
    ...
    //We manually created exceptions. This can be done without repeats
     if ( ! empty ($exceptions ) ) {
       ...
       for ( $i = 0; $i < $exceptcnt; $i++ ) {
         $sql = 'INSERT INTO webcal_entry_exceptions ( cal_id, 
            ...
            $msg .= ' ' . $WC->getFullName () .  ".\n" .
              ...
              if ( $timetype == 'T')  {
                ...
                if ( getpref ( 'GENERAL_USE_GMT' ) ) {
                  ...
                  $msg .= display_time ( $eventstart, 3 );
                } else {
                  // Display time in server's timezone
                  $msg .= display_time ( $eventstart, 6 );              
                }
              } 
              // Add Site Extra Date if permitted
...
if ( empty ( $error )  && empty ( $mailerError ) ) {
  ...
  $xdate = sprintf ( "%04d%02d%02d", $entry_year, $entry_month, $entry_day );
...
<?php
  if (  $timetype == 'A' ) {
    ...
    echo display_time ( $eventstart, 0 );
    if ( $duration > 0 ) {
      echo "-" . display_time ( $eventstart + ( $duration * 60 ), 0 );
    }
  }
