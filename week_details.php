...
function print_detailed_entry ( $event, $date ) {
  ...
  if ( ! $WC->isLogin( $event->getLogin() ) && strlen ( $event->getLogin() ) ) {
 ...
 if ( $event->isAllDay() ) {
  ...
  $timestr = smarty_modifier_display_time ( $event->getDateTime() ) .
   ' - ' . smarty_modifier_display_time ( $event->getEndDate( 'YmdHis' ) );
  echo $timestr . '&raquo;&nbsp;';
 }
  ...
  } elseif ( ! $WC->isLogin( $event->getLogin() ) && 
    $event->getAccess() == 'R' && strlen ( $event->getLogin() ) ) {
    $PN = $PD = '(' . translate ( 'Private' ) . ')';
  } elseif ( ! $WC->isLogin( $event->getLogin() ) && 
    strlen ( $event->getLogin() ) ) {
