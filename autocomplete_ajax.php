...
*/
  ...
  foreach ( $words as $i ) {
    ...
    $res = dbi_execute (  'SELECT we.cal_id, we.cal_name, we.cal_date, weu.cal_login '
    ...
    // but, will only search the first 1kb of the description.
      . 'AND ( UPPER( we.cal_name ) LIKE UPPER( ? ) OR UPPER( '
      . ( strcmp ( $GLOBALS['db_type'], 'mssql' ) == 0
        ? 'CAST ( we.cal_description AS varchar (1024) )'
        : 'we.cal_description' )
      . ' ) LIKE UPPER( ? ) ) ORDER BY we.cal_date ' . $order
      . ', we.cal_name', array ( $login, '%' . $i . '%', '%' . $i . '%' ) );
    if ( $res ) {
