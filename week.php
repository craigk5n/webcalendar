...
<td style="text-align:left;"><a href="week.php?<?php echo $u_url; ?>date=<?php echo date("Ymd", $prev ) . $caturl;?>"><img src="leftarrow.gif" width="36" height="32" border="0" alt="<?php etranslate("Previous")?>"></a></td>
<?php } ?>
...
<?php if ( empty ( $friendly ) || ! $friendly ) { ?>
<td align="right" style="text-align:right;"><a href="week.php?<?php echo $u_url;?>date=<?php echo date ("Ymd", $next ) . $caturl;?>"><img src="rightarrow.gif" width="36" height="32" border="0" alt="<?php etranslate("Next")?>"></a></td>
<?php } ?>
</tr>
</table>
...
for ( $d = $start_ind; $d < $end_ind; $d++ ) {
  ...
  if ( ! empty ( $hour_arr[9999] ) && strlen ( $hour_arr[9999] ) ) {
    $untimed[$d] = "<td width=\"12%\" style=\"width:12%; background-color:$TODAYCELLBG;\"><font size=\"-1\">$hour_arr[9999]</font></td>\n";
    $untimed_found = true;
  }
...
if ( $untimed_found ) {
  echo "<tr><td class=\"tableheader\" width=\"12%\" style=\"width:12%; background-color:$THBG;\">&nbsp;</td>";
  for ( $d = $start_ind; $d < $end_ind; $d++ ) {
    ...
    if ( ! empty ( $untimed[$d] ) && strlen ( $untimed[$d] ) )
      ...
      echo "<td width=\"12%\" style=\"width:12%; background-color:$color;\">&nbsp;</td>";
  }
  echo "</tr>\n";
}
...
for ( $i = $first_slot; $i <= $last_slot; $i++ ) {
  ...
  echo "<tr><th width=\"12%\" class=\"tableheader\" style=\"vertical-align:top; width:12%; background-color:$THBG; height:40; color:$THFG;\">" .  $time . "</th>\n";
  for ( $d = $start_ind; $d < $end_ind; $d++ ) {
    $thiswday = date ( "w", $days[$d] );
    $is_weekend = ( $thiswday == 0 || $thiswday == 6 );
    if ( empty ( $WEEKENDBG ) )
    ...
    if ( $rowspan_day[$d] > 1 ) {
      ...
      if ( ! empty ( $save_hour_arr[$d][$i] ) )
        echo "<td width=\"12%\" valign=\"top\" style=\"vertical-align:top; width:12%; background-color:$TODAYCELLBG;\"><font size=\"-1\">" .
          $save_hour_arr[$d][$i] . "</font></td>";
      $rowspan_day[$d]--;
    } else {
      if ( empty ( $save_hour_arr[$d][$i] ) ) {
        echo "<td width=\"12%\" valign=\"top\" style=\"vertical-align:top; width:12%; background-color:$color;\">";
        if ( empty ( $friendly ) && $can_add )
        ...
        if ( $rowspan_day[$d] > 1 ) {
          echo "<td width=\"12%\" valign=\"top\" style=\"vertical-align:top; width:12%; background-color:$TODAYCELLBG;\" rowspan=\"$rowspan_day[$d]\">";
          if ( empty ( $friendly ) && $can_add )
            echo html_for_add_icon (  date ( "Ymd", $days[$d] ), $time_h, $time_m, $user );
