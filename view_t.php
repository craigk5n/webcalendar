...
function print_entry_timebar ( $event, $date ) {
  ...
  $insidespan = false;
  $ret = '';
  $time_only = access_user_calendar ( 'time', $event->getLoginId() );
  $can_access = access_user_calendar ( 'view', $event->getLoginId(), '', 
  ...
  $ret .= ($start_padding > 0 ?  "<td class=\"alignR\" colspan=\"$start_padding\">": '' );
  if ( $pos > 0 ) {
    if ( ! $event->isUntimed() ) {
      ...
      if ( $pos > 1 ) {
        $ret .= "$fill</td>\n";
        $ret .= "<td class=\"alignL\" colspan=\"$end_padding\">";
      }
  ...
  if ( $event->isAllDay() ) {
    ...
    if ( $event->getDuration() > 0 ) {
      $timestr .= ' - ' 
			. display_time ( $event->getEndDate( 'YmdHis' ), 2 );
    }
  }
  ...
  if ( $pos < 2 ) {
    ...
    $ret .= ($end_padding > 1 ? "<td class=\"alignL\" colspan=\"$end_padding\">&nbsp;</td>\n": '' );
  }
