...
<input name="formtype" type="hidden" value="edituser">
<?php
 if ( empty ( $user ) ) {
  echo '<input name="add" type="hidden" value="1">' . "\n";
 }
?>
<table>
 <tr><td>
  ...
  <?php
    if ( ! empty ( $user ) ) {
      ...
      echo '<input id="username" maxlength="25" name="user" size="25" type="text" onchange="check_name();">' . "\n";
    }
?>
 </td></tr>
 <tr><td>
...
<?php if ( empty ( $user ) && ! $use_http_auth && $user_can_update_password ) { ?>
 <tr><td>
  ...
  <input id="pass1" name="upassword1" size="15" type="password" value="">
 </td></tr>
 <tr><td>
  <label for="pass2"><?php etranslate ( 'Password' )?> (<?php
  etranslate ( 'again' )?>):</label></td><td>
  <input id="pass2" name="upassword2" size="15" type="password" value="">
 </td></tr>
<?php }
...
<?php }  else if ( $is_admin ) {  ?>
  <input name="uis_admin" type="hidden" value="Y">
  <input name="u_enabled" type="hidden" value="Y">
<?php } //end if ($is_admin ) ?>
 <tr><td colspan="2">
  <?php if ( $DEMO_MODE == 'Y' ) { ?>
   <input type="button" value="<?php etranslate ( 'Save' )?>" onclick="alert('<?php
  etranslate ( 'Disabled for demo', 1 )?>')">
...
<input name="formtype" type="hidden" value="setpassword">
<?php if ( $is_admin ) { ?>
...
<table>
 <tr><td>
  ...
  <input id="newpass1" name="upassword1" size="15" type="password">
 </td></tr>
 <tr><td>
  <label for="newpass2"><?php etranslate ( 'New Password' )?> (<?php
   etranslate ( 'again' )?>):</label></td><td>
  <input id="newpass2" name="upassword2" size="15" type="password">
 </td></tr>
 <tr><td colspan="2">
  <?php if ( $DEMO_MODE == 'Y' ) { ?>
