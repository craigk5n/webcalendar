<?php
/**
 * The Doc class represents an attachment/comment.  This class
 * provides a convenient way to load and access both attachments
 * and comments without the caller needing to know about the underlying
 * database schema.
 *
...
/**
 * A Document object.  This can represent an event attachment or
 * comment.  Currently, a Doc must be associated with an event, but
 * this may change down the road.  We could use this object to
 * support category or user icons, for example.
 */
class Doc {
  /**
   * The unique id
  ...
  /**
    * Provide a list of comma-separated column names for use in a SQL
    * query to the webcal_blob table.
    */
  function getColumnList ()
  {
    return 'cal_blob_id, cal_id, cal_login_id, cal_name, cal_description, ' .
      'cal_size, cal_mime_type, cal_type, cal_mod_date';
  }

  /**
    * Create a SQL query to load attachment meta data for the
    * specified event id.
    * @param int $event_id  The event id
    * @return string  The SQL text
    * @access public
    */
  function getAttachmentsSQL ( $event_id )
  {
    return getSQL ( $event_id, 'A' );
  }


  /**
    * Create a SQL query to load comment meta data for the specified event id.
    * @param int $event_id  The event id
    * @return string  The SQL text
    * @access public
    */
  function getCommentsSQL ( $event_id )
  {
    return getSQL ( $event_id, 'C' );
  }
  ...
  function getSQL ( $event_id, $type='A' )
  {
    return 'SELECT ' . Doc::getColumnList () .
      ' FROM webcal_blob WHERE cal_id = ' . $event_id .
      " AND cal_type = '" . $type . "'" .
      ' ORDER BY cal_mod_date DESC';
  }

  /**
    * Generate the SQL to load a single doc specified by its
    * unique ID
  ...
  function getSummary () {
    ...
    if ( $this->_size < 1024 )
      ...
      $ret .= sprintf ( " %.1f", ( $this->_size / 1024 ) ) .
        translate ( 'kb' );
    else
      $ret .= sprintf ( " %.1f", ( $this->_size / 1024 * 1024 ) )
        . translate ( 'Mb' );
    $ret .= ', ' .
      date_to_str ( $this->_mod_date, '', false, true ) . ' )';
    return $ret;
  }

  /**
    * Get the binary data of the document.
    * This is NOT cached and is only loaded when this function is
    * called.  Repeated calls to this function will make repeated
    * calls to the database.
    * @return mixed  binary data of the blob
    */
  function getData () {
    return dbi_get_blob ( 'webcal_blob', 'cal_blob',
          'cal_blob_id = ' . $this->_blob_id );
  }
