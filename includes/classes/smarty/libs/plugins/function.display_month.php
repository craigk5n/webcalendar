...
function smarty_function_display_month ( $params, &$smarty ) {
  ...
  $smarty->assign('display_weeknumber', $display_weeknumber ); 
  ...
  for ( $i = $wkstart; date ( 'Ymd', $i + 43200 ) <= $monthend;
    ...
    for ( $j = 0; $j < 7; $j++ ) {
      ...
      if ( $currMonth || $display_all_days_in_month ) {
        ...
        $ret_events = '';
        if ( ! $demo )
          $ret_events = month_labels ( $dateYmd );
        else {
...
function month_labels ( $date ) {
  ...
  $urlparams = $WC->getUserUrl('?') . $WC->getCatUrl();
  $urlparams = ( $urlparams ? $urlparams . '&amp;' : '?' );
  
  $ret = ( $WC->isAdmin() || ( ! _WC_READONLY && ! $is_nonuser   ) ? '
    <a title="' . $newEntryStr . '" href="edit_entry.php' . $urlparams
    . 'date=' . $date . '"><img src="images/new.gif" alt="' 
