...
function smarty_function_week_glance ( $params, &$smarty ) {
  ...
  if ( ! $event->isUntimed() && $getCalTypeName != 'task' ) {
    if ( $show_time )
      $hour_arr[$ind] .= smarty_modifier_display_time ( $getDatetime )
       . ( $time_only == 'Y' ? '' : getPref ( 'TIME_SPACER' ) );

    $timestr = smarty_modifier_display_time ( $getDatetime );
    if ( $event->getDuration () > 0 ) {
      $end_time = date ( 'His', $event->getEndDate () );
      $timestr .= '-' . smarty_modifier_display_time ( $event->getEndDate (), $DISPLAY_TZ );
