...
class RepeatingEvent extends Event {
 ...
 /**
   ...
   * @access public
   */
  function RepeatingEvent ( $name, $description, $date, $eid, $rmtaddr,
    $priority, $access, $duration, $status, $owner, $login, $category,
    $cal_type, $location, $url, $due_date, $percent, $moddate, 
    $completed, $parent, $rpt_type, $end, $frequency, $rpt_bymonth,
    $rpt_bymonthday, $rpt_byday, $rpt_bysetpos, $rpt_byweekno,
    $rpt_byyearday, $rpt_wkst, $rpt_count,
    $exceptions, $inclusions, $rpt_all_dates ) {

    /* Silly PHP4 hack */
    $parent = get_parent_class ( $this );

    parent::$parent ( $name, $description, $date, $eid, $rmtaddr,
      $priority, $access, $duration, $status, $owner, $login, $category,
      $cal_type, $location, $url, $due_date, $percent, $moddate, $completed, $parent );
  ...
  *
   ...
   * @param int $eid ID of exception 
   *
   * @access public
   */
  function addRepeatException ( $exception, $eid ) {
    $this->_repeatExceptions[] = $exception . $eid;
  }
  ...
  /**
   * Checks to see if advanced repeat options are used
   *
   * @access public
   */
  function isExpertMode () {
	  $expert = ( $this->_repeatCount || $this->_repeatByYearDay 
		  || $this->_repeatByWeekNo 
			|| $this->_repeatBySetPos || $this->_repeatByMonthDay 
		  || $this->_repeatByMonth || $this->_repeatByDay );
    return $expert;
  } 
	
  /**
   * Checks to see if this event is weekday only
   *
   * @access public
   */
  function isWeekdaysOnly () {
	 global $WC;
	  //use may have set weekend diffent than SA/SU
	  $weekendstart = getPref ( 'WEEKEND_START' );
		$weekstart = ( $weekendstart + 2 ) %7;
		$weekend = ( $weekstart + 4 ) %7;
    if ( $weekstart > $weekend )
		  list($weekstart, $weekend) = array($weekend, $weekstart);
		for ( $i=$weekstart; $i<=$weekend; $i++ ) {
		  $weekdayarray[] = $WC->byday_names[$i];
		}
	  $weekdays = ( $this->_repeatType == 'daily' &&
				$this->_repeatByDay == $weekdayarray ? true : false );
    return $weekdays;
  } 
}
?>
