...
'/^init\.php$/' =>
  ...
  function _initInitSecondPhase() {
    ...
    if ( getPref ( 'CATEGORIES_ENABLED', 2 ) )
      $this->_cat_id = $this->getValue ( 'cat_id', '[\-0-9]+' );
  ...
  if (  CALTYPE == false || CALTYPE == 'month' ) {
    ...
    $this->_startdate = mktime ( 0, 0, 0, 
      $this->prevmonth, 1, $this->prevyear );
    $this->_enddate = mktime ( 23, 59, 59, 
      $this->nextmonth + 1, 0, $this->nextyear );
    } else {
    $this->_startdate = mktime ( 0, 0, 0, 
...
//    if  ( getPref ('DISPLAY_SM_MONTH' ) && getPref ( 'BOLD_DAYS_IN_YEAR' ) ) {
//    $this->_startdate = get_weekday_before ( $this->thisyear, $this->thismonth );
 //   $this->_enddate = mktime ( 23, 59, 59, 
//      $this->thismonth + 2, 0, $this->thisyear );
//    } else {
    $this->_startdate = get_weekday_before ( $this->thisyear, 
      $this->thismonth, $this->thisday +1 );
    $this->_enddate = $this->_startdate + ( ONE_DAY * ( 
      getPref ('DISPLAY_WEEKENDS') ? 5 : 7 ) );
//    }
  } else if ( CALTYPE == 'year' ) {
    ...
    */
		...
		if ( ! _WC_HTTP_AUTH && !_WC_SINGLE_USER ) {
		  ...
		  $this->_logout_url = $this->_login_url . '?action=logout';
			...
			if ( substr( _WC_USER_INC, 13, 7 ) == 'UserApp' ) {
  ...
  function _initValidate() {
    ...
    if ( _WC_SINGLE_USER ) {
      ...
      if ( _WC_HTTP_AUTH ) {
      ...
      } elseif ( substr( _WC_USER_INC, 13, 7 ) == 'UserApp' ) {
        ...
        //Test  for ESSION first, then decode COOKIE if needed
        @session_start ();
 ...
 */
 function _decode_string ( $instr ) {
   ...
   */  
  function catId () {
    return ( ! empty ( $this->_cat_id) ? $this->_cat_id : -99 );
  }
   ...
   * @return string The user url 
   */
  function getCatUrl ( $seperator = '&amp;') {
    return ( ! empty ( $this->_cat_id ) && $this->_cat_id > -99
    ? $seperator . 'cat_id='. $this->_cat_id : false );
  }  
   /**
