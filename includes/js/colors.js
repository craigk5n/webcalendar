...
// Predefined Colors
var pcoc = wc_getCookie('webcalendar_custom_colors'),
precol,
...
addLoadListener(function () {
  var tmp = document.getElementById('colorchoices').getElementsByTagName('td');

  for (var i = 48; i > 0; i--) {
    tmp[i].bgColor = colorList[i];

    attachEventListener(tmp[i], 'click', function () {
      setCol(colorList[i]);
    });
  }
  tmp = document.getElementById('colorcustom').getElementsByTagName('td');
  ...
  attachEventListener(document.getElementById('colorpic'), 'click', function () {
    setFromImage(event);
  });
  attachEventListener(document.getElementById('htmlcolor'), 'change', setFromHTML);
  attachEventListener(document.getElementById('rgb_r'), 'change', setFromRGB);
  attachEventListener(document.getElementById('rgb_g'), 'change', setFromRGB);
  attachEventListener(document.getElementById('rgb_b'), 'change', setFromRGB);
  attachEventListener(document.getElementById('setSlide'), 'click', function () {
    setFromSlider(event);
  });
  tmp = getElementsByAttribute('type', 'button');

  attachEventListener(tmp[0], 'click', definePreColor);
  attachEventListener(tmp[1], 'click', function () {
    transferColor();
    window.close();
  });
  attachEventListener(tmp[2], 'click', window.close);
  ...
  currgb = [fromhex(curcol.substr(0, 2)), fromhex(curcol.substr(2, 2)),
    fromhex(curcol.substr(4, 2))];
...
function transferColor() {
  ...
  wc_setCookie('webcalendar_custom_colors', precol.join(','), expires);
  thisInput.value = '#' + document.getElementById('htmlcolor').value.toUpperCase();

  if (thisInput.onchange) {
    // This updates the color swatch for this color input.
    // It relies on the <input>s of the prefform having
...
function update() {
  ...
  for (var i = 0; i < 192; i++) {
    ...
    document.getElementById('sc' + (i + 1)).bgColor = '#' + tohex(rgb[0])
       + tohex(rgb[1]) + tohex(rgb[2]);
  }
}
function HSLtoRGB(h, s, l) {
  ...
  rm1 = 2.0 * l - rm2;
  return [ToRGB1(rm1, rm2, h + 120.0), ToRGB1(rm1, rm2, h),
...
function RGBtoHSL(r, g, b) {
  ...
  if (max == min) {
    ...
    // s
    s = Math.round(255 * (max - min) / (l < 128 ? (max + min) : (510 - max - min)));
