 ...
 * @return string
 */
function smarty_outputfilter_translate($source, &$smarty)
{

  preg_match_all("/\[translate\s\(\s'(.*)'\s\)\]/U", $source, $match_trans);
	
	preg_match_all("/\[tooltip\s\(\s'.*'\s\)\]/U", $source, $match_tool);
	
  foreach (  $match_trans[1] as $match ) {
	$pattern =  "/\[translate\s\(\s'" .$match. "'\s\)\]/U";
	  $source = preg_replace( $pattern, translate ( $match ),$source);
	}

  foreach (  $match_tool as $match ) {
	  $source = preg_replace( "/\[tooltip\s\(\s'.*'\s\)\]/U", tooltip ( $match),$source);	
	}			
	return $source; 
