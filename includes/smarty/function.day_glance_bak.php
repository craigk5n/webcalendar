...
function smarty_function_day_glance ( $params, &$smarty ) {
  ...
  for ( $i = $first_slot; $i <= $last_slot; $i++ ) {
    ...
    $displayTime = smarty_modifier_display_time ( mktime ( $time_h, $time_m , 0, 
...
function html_for_event_day_at_a_glance ( $event, $date ) {
  ...
  if ( $time >= 0 && ! $isAllDay && $getCalTypeName != 'task' ) {
    $end_timestr = '-' . smarty_modifier_display_time ( $event->getEndDate () );
    $popup_timestr = smarty_modifier_display_time ( $event->getDate () );
