...
function dbi_connect ( $host, $login, $password, $database, $lazy = true ) {
  ...
  } elseif ( strcmp ( $GLOBALS['db_type'], 'mysqli' ) == 0 ) 
  {
    ...
    {      
		...
		# FG: 11.02.2018 - If the character set used in the mysql/mariadb connection is not uf8 or a derivative the 
		#     character set will be changed to utf8.
		#     This avoids a lot of problems in data transfer (ical-formats) and the screen display.
		#     see issue xxx on GitHUB.
		# FG: 11.02.2018 - Just a variable to quickly allow debug output on the html-page. Set to a value bigger than
		#     0 will cause screen output of information useful for debugging.
		$fgdebugdbinfo = 0;
		$original_character_set = $c->character_set_name();
		if ($fgdebugdbinfo >= 1)
		{
			# FG: ##### Testausgabe des gesetzten Zeichensatzes
			echo "<p> Debug-Information: currently used database: --".$c->host_info."--</p>";
			# FG: ##### Testausgabe des gesetzten Zeichensatzes
			echo "<p> Debug-Information: currently used character set: --".$c->character_set_name()."--</p>";
		}
		if (strpos(strtolower($original_character_set),"utf") == false)
		{
			...
			if ($fgdebugdbinfo >= 1)
			{			
				if (!$resultOfCharsetChange)
				{
					echo "<hr/>";
					printf ("<p> Debug-Information: Error setting the character set to utf8 %s: %s<p>", $charsetName, $c->error);
					echo "<hr/>";
				}
				printf("<p> Debug-Information: Current character set used for database connection (should but utf-8): %s</p>", $c->character_set_name());
			}
		}
		return $c;
    } 
