 ...
 * in the webcal_site_extras table or the default values if no $eid is passed
 *
 ...
 *   - <var>cal_data</var>
 */
function get_site_extra_fields ( $eid='' ) {
  global $site_extras;
  $extras = array ();
  if ( $eid ) {
    $rows = dbi_get_cached_rows ( 'SELECT cal_name, cal_type, cal_date,
      cal_data FROM webcal_site_extras WHERE cal_id = ?', array ( $eid ) );
    if ( $rows ) {
