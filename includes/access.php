...
function access_get_function_description ( $function ) {
  switch ( $function ) {
    ...
    default:
      die_miserable_death ( str_replace ( 'XXX', $function, translate ( 'Invalid function id XXX' ) ) );
  }
}
...
function access_load_user_permissions ( $useCache = true ) {
  ...
  $res = dbi_execute ( 'SELECT cal_login, cal_other_user, cal_can_view, cal_can_edit, cal_can_approve, cal_can_email, cal_can_invite, cal_see_time_only
...
function access_get_viewable_users ( $user ) {
  ...
  foreach ( $access_other_cals as $i ) {
    if ( preg_match ( '/' . $user . "\.(\S+)/", $i, $matches ) )
      $ret[] = $matches[1];
  }
  return $ret;
}
 ...
 *
 * If no entry is found for the specified user, then look up the user '__default__'.
 * If still no info found, then return some default values.
 *
 * @param string $user User login
 *
...
function access_load_user_functions ( $user ) {
  ...
  foreach ( $users as $i ) {
    $res = dbi_execute ( 'SELECT cal_permissions
      FROM webcal_access_function
      WHERE cal_login = ?', array ( $i ) );
    assert ( '$res' );
  ...
  if ( ! empty ( $rets[$user] ) )
    $ret = $rets[$user];
  else if ( ! empty ( $rets['__default__'] ) )
...
function access_user_calendar ( $cal_can_xxx = '', $other_user, $cur_user = '',
  ...
  else if ( isset ( $access_other_cals[$key4][$cal_can_xxx] ) )
