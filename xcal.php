...
function export_get_attendee( $id, $export ) {
  ...
  while ( list ( $key, $row ) = each ( $entry_array ) ) {
    ...
    else 
    {
      ...
      {
      	// FG: 11.02.2018 - UTF-8 encoding should only be called in case the string is not 
      	//     already utf-8 encoded, otherwise the output results in errors.
      	$fguserattendee = $user['cal_firstname'] . ' ' .  $user['cal_lastname'];
      	if (!mb_check_encoding($fguserattendee, 'UTF-8'))
      	{
      		# FG: 11.02.2018 - just for debugging purposes, to see which kind of encoding 
      		#     was present
        	# $attendee[$count] .= ';CN="' . utf8_encode($fguserattendee) . '-enc"';
        	$attendee[$count] .= ';CN="' . utf8_encode($fguserattendee) . '"';
       	}
       	else
       	{
        	$attendee[$count] .= ';CN="' . $fguserattendee .'"';
       	}
      }
      if ( ! empty ( $user['cal_email'] ) )
