...
public class ReminderLoader extends Thread {
  ...
  public ReminderLoader ( WebCalendarClient client,
    ReminderDisplayer display, int reloadMinutes )
  {
    super ();
    this.client = client;
  ...
  public ReminderList doUpdate ()
  {
    try {
      ...
      client.getMessageDisplayer().showError ( "WebCalendar Error:\n" +
        e.getMessage() );
    } catch ( Exception e ) {
       ...
       if ( e.toString().indexOf ( "401" ) >= 0 ) {
         ...
         client.getMessageDisplayer().showError ( "Error getting data from server:\n" + e.toString() );
       }
    }
    return null;
  }
