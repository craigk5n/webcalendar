  ...
  if ( $res ) {
    while ( $row = dbi_fetch_row ( $res ) ) {
      ...
      if ( count ( $data ) && empty ( $errormsg ) ) {
        ...
        // import new events
        if ( $debug )
          echo "Importing events for: $calUser<br>From: $cal_url.<br>\n";
        ...
        if ( count ( $data ) == 0 )
          echo "<br>\n" . str_replace ( ['XXX','YYY'],[$cal_url,$calUser], translate ( 'No data returned from&#58; "XXX" for non-user calendar&#58; "YYY".' ) ) . "<br>\n";
      }
    }
    dbi_free_result ( $res );
  }
