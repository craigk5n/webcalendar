...
$smarty->assign ( 'areYouSureStr', $areYouSureStr );
...
$end_str = ( $item->getDuration () > 0
  ? '-' . smarty_modifier_display_time ( $item->getDate () + ( $item->getDuration () * 60 ) )
  : '' );
...
$event_date = ( $event_repeats && ! empty ( $date ) ? $date : $item->getDate () );

// Get category Info
if ( getPref ( 'CATEGORIES_ENABLED' ) ) {
  $categories = get_categories_by_id ( $eid,
...
$smarty->assign ( 'display_date', $display_date );
...
if ( $item->isRepeat () ) {
  $smarty->assign ('event_repeats', true );
	$smarty->assign ('editAllDatesStr', 
	  translate ( 'Edit repeating entry for all dates' ) );
	$smarty->assign ('deleteAllDatesStr', 
	  translate ( 'Delete repeating event for all dates' ) );
	$smarty->assign ('editThisDateStr', translate ( 'Edit entry for this date' ) );
	$smarty->assign ('deleteOnlyStr', 
...
$smarty->assign ('timeStr', ( $item->isAllDay() ? translate ( 'All day event' )
    : smarty_modifier_display_time ( $item->getDate (), ( empty ( $end_str ) ? 2 : 0 ) ) . $end_str ) );
...
$smarty->assign ( 'itemModDate', smarty_modifier_display_time ( $item->getModDate (), getPref ( 'GENERAL_USE_GMT' ) ?3 : 2 ) );
...
if ( ! _WC_READONLY && ! $is_my_event && $event_status != 'D' &&  ! $is_nonuser )
  $smarty->assign ( 'addToMineStr', translate ( 'Add to My Calendar' ) );

if ( ! $WC->login( '__public__' ) && count ( $allmails ) > 0 )
  $smarty->assign ( 'emailAllStr', translate ( 'Email all participants' ) );
...
if ( access_can_access_function ( ACCESS_EXPORT ) && ! $hide_details ) {
  $smarty->assign ( 'allowExport', true );
   $smarty->assign ( 'exportSelectStr', generate_export_select () );	
}

$smarty->assign ( 'copyStr', translate ( 'Copy entry' ) );
