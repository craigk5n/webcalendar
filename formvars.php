  ...
  if ( is_array ( $instr ) ) {
    for ( $j = 0; $j < count ( $instr ); $j++ ) {
      for ( $i = 0; $i < count ( $bannedTags ) && ! $failed; $i++ ) {
        ...
        //
        /* FG -  26.12.2017 PHP7.x - prevent deprecated warning for /e in preg_replace
            #e is deprecated since PHP 5.5 and ignored from PHP 7.x onwards 
			Previous task:
				It used to tell the function preg_match that we do not want to
				replace the Esscape code with the literal string chr(hexdec('\\1'))
				but with the character with the found escape code.
				Sticking ot the example: 
					'\x3c is found. -> match is 3c -> hexdec converts 3c to 195 ->
					which in turn is replaced with the unicode character 'Ä' (German A Umlaut)
			intended purpose (my guess)
				make sure that the following search for banned tags will not fail if the 
				banned tag is written with Escape codes.
			Previous code:
