...
- **`php bin/webcal.php user list`** shows each account with its name, whether it is an administrator, whether it is enabled, and the address it receives mail at. Reading state was the last thing here that still meant writing a throwaway PHP script, and the mail column is not incidental: a test message that never arrived was traced by noticing that reminders reached the same address, so the fault had to be in the message rather than the route

- **`make sandbox`**, a disposable installation in a container, for trying anything that writes. A WebCalendar checkout is frequently also a live install, and bind mounting the tree into a container and installing into it is how `tests/run-*-install-tests.sh` deleted a live `includes/settings.php` and took a calendar down on 2026-09-24 — so containerisation is not what makes this safe. Two independent properties are: the working copy is mounted **read-only**, so the kernel refuses the write, and `WEBCALENDAR_USE_ENV=true` means `do_config()` takes the database settings from the environment and never opens `includes/settings.php`, so a live configuration in the mount is ignored rather than merely protected. The database lives in a named docker volume, so nothing appears in the working copy either. `make sandbox-cli CMD="..."` runs `bin/webcal.php` inside it, `make sandbox-reset` discards the data, `make sandbox-clean` removes everything. `SANDBOX_PORT` overrides the port, which 8081 needs often enough that the target checks it and says so. `tests/SandboxIsolationTest.php` asserts both isolation properties, each verified by breaking it
...
- **The function codemap records what each function declares `global`, and covers `includes/xcal.php`.** These files pass data through globals, so a signature says little about what a function touches: `export_get_event_entry()` takes two arguments and reads thirteen globals, and **112 of the 219 indexed functions** declare at least one. Knowing that previously meant reading the function. `includes/xcal.php` joins `functions.php` and `dbi4php.php` as a source — 39 more functions, and every export and import path runs through it. `tests/CodemapTest.php` now also runs the generator's own `--check` mode, which regenerates and compares byte for byte, so the map cannot fall behind in any column, including ones added later, without a second implementation to drift from the first

- **The structural tests read source as code through one tested helper instead of fifteen private copies.** They search source files for the mechanism they guard — a condition, a required argument, a mount option — and the file almost always explains that mechanism in a comment right beside it, so a search of the raw text is satisfied by the explanation. That defect appeared six times in a single day, four of them in guards that had looked fine for months: `DestructiveTestGuardTest` passed with its refusal replaced by `if false`, and its ordering check compared against a comment so could never fail at all; `WizardRemovableTest` reported a file as depending on `wizard/` because a comment described the dependency in the opposite direction; `ActivityLogConstantsTest` reported a constant as undefined that was a typo in a docblock. `tests/SourceText.php` now provides `php()`, `phpFunction()`, `phpFunctionBody()`, `shell()`, `yaml()` and `python()`, and `tests/SourceTextTest.php` tests each against the case that produced it — fourteen assertions where there were none. Eight copies of a brace-walking function extractor and seven of a comment stripper are gone. Every one of the fourteen affected guards was re-verified by reintroducing the defect it exists to catch

- **The wizard's Selenium tests wait for conditions instead of the clock.** Each of `tests/web-install-{mysql,postgresql,sqlite}.py` carried fifteen `time.sleep()` calls. Five of those are the poll interval of a bounded wait loop and are correct; the other ten were unconditional, and four of them sat immediately before an explicit wait that already covered the same thing. They are now a `readyState` wait after a navigation, a wait for the text the following assertion looks for, or nothing. Measured against an isolated copy of the tree: SQLite 111.8s to 86.3s, MySQL 125.5s to 100.2s, PostgreSQL 275.2s to 248.3s — about 25 seconds off each suite, with four consecutive SQLite runs inside a 2.6-second spread. `tests/SeleniumWaitsTest.php` fails the build if a fixed sleep returns directly after a navigation

- **The three Selenium wizard jobs build the web image once, with a layer cache.** Each was rebuilding it from the same Dockerfile, recompiling the same PHP extensions, for 48 seconds of a 271-second job. The workflow now builds it with buildx against the GitHub Actions cache and loads it under a fixed tag, and the compose files name that tag, so compose finds it present and skips its own build — verified against a throwaway compose file: with the tagged image present the build is skipped entirely and the container still runs, and with the tag drifted it builds again. `tests/DockerImageTagTest.php` fails the build if the workflow's tag and the compose files' `image:` keys ever diverge, because nothing else would: compose would simply rebuild and the pipeline would stay green

- `tests/DockerReferencesTest.php` no longer reports a glob as a missing file. Its path pattern stops at the `*`, so `docker/docker-compose-test-*.yml` written in a comment matched as `docker/docker-compose-test-` and failed the build. It now checks whether a glob character follows the match, and still catches a genuinely missing path
...
- **The release workflow cancelled its own test jobs.** `release.yml` calls five other workflows, and four of those declare `concurrency: group: ${{ github.workflow }}-${{ github.ref }}`, added earlier in this release to stop superseded pull request runs from occupying runners. Inside a called workflow that expression is the name of the *caller*, so `ci.yml`, `phpstan.yml` and `test-web-wizard.yml` all evaluated it to `Create Release-refs/heads/release` and shared one group. GitHub allows one pending run per group and cancels the older pending one, so cutting v1.9.24 ended with PHPStan cancelled one second in having run no steps, `build` skipped for an unmet `needs`, and nothing published. Which of the three loses depends on scheduling order, which is why it had not happened before. Each workflow now uses a literal prefix; pull request behaviour is unchanged, because on a pull request there is no caller and the group was already the workflow's own name. `tests/ReusableWorkflowConcurrencyTest.php` fails the build when a called workflow keys its group on `github.workflow`, or when two callees of one caller resolve to the same group, and reads the YAML with comments stripped -- the fix carries a comment naming the expression it forbids, which a text search would have accepted

- **`MCP_WRITE_ACCESS` was documented only in `docs/mcp-server.md`.** It is the setting that decides whether an AI assistant can create, change and delete events, and it was missing from the configuration reference, where `MCP_SERVER_ENABLED` and `MCP_RATE_LIMIT` both appear, and from `docs/security.md`'s MCP hardening advice, where its two siblings also appear. An administrator hardening an installation from that page would not have learned that the read-only default exists or that it can be turned off

- **The documented password recovery was the insecure one.** `docs/troubleshooting.md` answered "cannot log in" with `UPDATE webcal_user SET cal_passwd = MD5('newpassword')`. That does still work — `user_valid_login()` reads a 32-character hexadecimal `cal_passwd` as a legacy MD5 hash and re-hashes it with bcrypt on the next successful login — but it leaves an unsalted MD5 in the table until then, reversible by anyone who can read `webcal_user` in the meantime. It now leads with `php bin/webcal.php user reset-password`, which has existed since earlier in this release and was not mentioned, and keeps the SQL as a fallback with that caveat stated

- **`bump_version.sh` did not update three version stamps that the build now checks.** `README.md`'s badge, `docs/WebCalendar-Database.md`'s `**Version:**` line and `CLAUDE.md`'s overview line are all asserted against `includes/config.php` by `tests/DocumentedVersionTest.php`, added earlier in this release after the badge was found advertising a version seven releases old. Bumping with the script alone would therefore have turned the build red on the next release — the test caught the drift but nothing closed the loop that produced it. The script now updates all three, `CLAUDE.md` only where it exists, since it is not tracked

- **`reset` left the import bookkeeping behind, so importing the same file again failed.** `php bin/webcal.php reset --force` emptied fourteen tables but not `webcal_import_data`, whose primary key is `(cal_id, cal_login)`. New events take `MAX(cal_id) + 1` from `webcal_entry` (`includes/xcal.php:1433`), so emptying that table restarts the ids at 1 while the leftover rows still claim them, and the next import collided on the first event — a fatal database error, which before the exit-status fix below reported success. `webcal_site_extras` was missed the same way, and per-event custom fields keyed on a recycled `cal_id` reattach to whichever event next takes it. `webcal_import` is now cleared too, since its rows are the parents of the `webcal_import_data` ones and nothing else refers to them once the entries are gone. Fixed, and verified as the sequence that failed: import three events, `reset --force`, import the same file again, three imported and no error. `tests/DevSeederTest.php` derives the list it requires from `wizard/shared/tables-sqlite3.php` — every table whose definition mentions `cal_id` must appear in a `DELETE` — so a table added to the schema later cannot be forgotten here without failing the build

- **A fatal database error reported success.** `dbi_fatal_error()` ended in a bare `exit`, which is status **0**, so every fatal database error told the caller it had worked. Demonstrated by renaming `webcal_reminders` in a sandbox and importing three events: one arrived, the summary never printed, and `php bin/webcal.php import` exited 0 — a cron entry or a script would have recorded a clean run against a half-loaded calendar. `tools/send_reminders.php` had the same problem, reporting success to cron when it had never connected. It now exits 1. **This is a behaviour change for anything watching those exit codes: failures that were silent will now be reported.** Under CLI the message also goes to standard error as plain text rather than as an HTML block on standard output, where `export` is writing an iCalendar document and `db dump` is writing SQL — an error there was written into the file as though it were data

- **The `user` command was unavailable on LDAP, IMAP, NIS and Joomla installations.** The refusal for those backends sat in the dispatch, before the action was looked at, so it applied to everything under `user` — including a read-only listing, on exactly the installations where an administrator most needs to see which accounts exist and cannot simply reset a password to get in. It now belongs to `reset-password`, which is the only part that writes a password column, and `user list` reports which backend owns authentication so nobody is surprised by the refusal afterwards

- **The documentation advertised a version seven releases old, and understated what the MCP server can do.** `README.md`'s badge said v1.9.16 against a program version of v1.9.23, its roadmap still labelled v1.9.16 "(Current)", `docs/admin-guide.md` was titled for v1.9.16 and ships, and `docs/WebCalendar-Database.md` said v1.9.13. Worse for anything reading the docs to decide what it can do: `docs/mcp-server.md` described **four** tools where `mcp.php` registers **nine**, so `add_recurring_event`, `update_event`, `delete_event`, `get_availability` and `check_conflicts` were undocumented, `add_event`'s `time` parameter was missing, and nothing mentioned that writing needs `MCP_WRITE_ACCESS` enabled in **Admin > Settings**. All now documented, and the admin guide lost its version stamp rather than gaining a rule to keep it current — a general guide gains nothing from one, and stamping it guarantees the drift. `tests/DocumentedVersionTest.php` locks the remaining version claims and the tool list to the code, taking the tool list from the `#[McpTool]` attributes rather than from a list someone maintains by hand; it found two of those five tools that a search for known names had missed

- **A broken install path on MySQL or PostgreSQL is no longer reported as a pass.** Both Selenium wizard jobs carried `continue-on-error: true`, under a TODO calling the harness flaky, which made them decorative: the checks were green whatever happened inside them. The record does not support the TODO. Across thirty workflow runs the only failures were two runs on one branch where **all three** jobs failed together, each with `login failed with both passwords` — a real defect, and exactly what these tests exist to catch. The SQLite job was already blocking and failed in both of those runs, so the build went red regardless: making the other two blocking would have added no red builds at all. The mechanism the TODO cited was the fixed sleeps, and those are gone. `tests/SeleniumWaitsTest.php` fails the build if a wizard job is silenced again
